...
//   ParameterNode,
//   Reference,
// } from "../types/app_legacy.types";
// import { ColumnRole, ColumnAggregationType } from "../types/enums";
...
// /**
//  * Finds metadata record for a column
//  */
// function findMetadataForColumn(
//   datasourceName: string,
//   columnName: string,
//   metadataRecords: TWBMetadataRecord[]
// ): TWBMetadataRecord | undefined {
//   return metadataRecords.find(
//     (record) =>
//       record["@_class"] === "column" &&
//       record["parent-name"] === `[${datasourceName}]` &&
//       record["local-name"] === columnName
//   );
// }
...
//     // Get columns from both locations
//     const directColumns = ds.column ? ensureArray(ds.column) : [];
//     const relations = ensureArray(ds.connection?.relation);
//     const relationColumns = relations.flatMap((r) =>
//       r.columns ? ensureArray(r.columns.column) : []
//     );

//     console.log(relationColumns);
//     const metadataRecords = ds["metadata-records"]?.["metadata-record"]
//       ? ensureArray(ds["metadata-records"]["metadata-record"])
//       : [];

//     // Combine and process all columns
//     [...directColumns, ...relationColumns].forEach((col) => {
...
//         const metadata = findMetadataForColumn(
//           ds["@_name"],
//           name,
//           metadataRecords
//         );

//         // Create column node with metadata
...
//           role,
//           aggregation:
//             (metadata?.aggregation?.toLowerCase() as ColumnAggregationType) ||
//             ColumnAggregationType.None,
//           ordinal: col["@_ordinal"]
//             ? parseInt(col["@_ordinal"], 10)
//             : undefined,
//           remoteAlias: metadata?.["remote-alias"] || name,
//           remoteName: metadata?.["remote-name"] || name,
//           remoteType: metadata?.["remote-type"] || "",
//           precision: metadata?.precision,
//           containsNull: metadata?.["contains-null"] || false,
//           metadata: metadata
//             ? {
//                 remoteName: metadata["remote-name"],
//                 remoteType: metadata["remote-type"],
//                 localName: metadata["local-name"],
//                 localType: metadata["local-type"],
//                 parentName: metadata["parent-name"],
//                 remoteAlias: metadata["remote-alias"],
...
//                   ? {
//                       flag: metadata.collation["@_flag"],
//                       name: metadata.collation["@_name"],
//                     }
//                   : undefined,
//                 attributes: metadata.attributes?.attribute.map((attr) => ({
//                   datatype: attr["@_datatype"],
//                   name: attr["@_name"],
//                   value: attr["@_value"],
//                 })),
//               }
...
//       } else if (isRelationColumn(col)) {
//         // Create column node for relation column
//         const colNode: ColumnNode = {
//           id,
//           name,
//           displayName,
...
//           caption,
//           dataType: col["@_datatype"],
//           role: role || ColumnRole.Dimension,
//           aggregation:
//             (col[
//               "@_aggregation"
//             ]?.toLocaleLowerCase() as ColumnAggregationType) ||
//             ColumnAggregationType.None,
//           ordinal: col["@_ordinal"]
//             ? parseInt(col["@_ordinal"], 10)
//             : undefined,
